...
class Config:
    """Central configuration for training, local API runtime, and Render."""

    base_dir: Path = BASE_DIR
    dataset_dir: Path = BASE_DIR / "dataset"
    models_dir: Path = BASE_DIR / "models"
    logs_dir: Path = BASE_DIR / "logs"
    database_path: Path = Path(os.getenv("DATABASE_PATH", str(BASE_DIR / "pulse_diagnosis.db")))
    dataset_path: Path = Path(os.getenv("DATASET_PATH", str(BASE_DIR / "dataset" / "pulse_features.csv")))
    model_path: Path = Path(os.getenv("MODEL_PATH", str(BASE_DIR / "models" / "model.pkl")))
    scaler_path: Path = Path(os.getenv("SCALER_PATH", str(BASE_DIR / "models" / "scaler.pkl")))
    metadata_path: Path = Path(os.getenv("MODEL_METADATA_PATH", str(BASE_DIR / "models" / "model_metadata.json")))
    metrics_path: Path = BASE_DIR / "models" / "metrics.json"
    confusion_matrix_path: Path = BASE_DIR / "models" / "confusion_matrix.png"
    roc_curve_path: Path = BASE_DIR / "models" / "roc_curve.png"
    feature_importance_path: Path = BASE_DIR / "models" / "feature_importance.png"
    log_file: Path = BASE_DIR / "logs" / "pulse_diagnosis.log"
...
def ensure_directories(config: Config = Config()) -> None:
    """Create runtime directories required by training and API execution."""
